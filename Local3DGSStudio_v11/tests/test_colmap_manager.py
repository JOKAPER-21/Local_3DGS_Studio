...
def test_feature_extraction_command_cpu(tmp_path: Path):
    ...
    assert str(manager.active_image_source_path()) in command.colmap_args
...
# -- architecture: no persistent colmap/vNN/images junction -----------------


def test_colmap_command_uses_raw_data_images_directly(tmp_path: Path):
    """Per the current architecture, COLMAP's --image_path must point
    directly at rawData/images/vNN -- never at a colmap/vNN/images
    junction (which no longer exists)."""
    project_manager = _open_project(tmp_path)
    manager = COLMAPManager(project_manager, COLMAPConfig(executable_path="/usr/bin/colmap"))

    feature_command = manager.build_feature_extraction_command()
    mapper_command = manager.build_mapper_command()

    expected_image_path = str(project_manager.current.resolver.images)
    assert expected_image_path in feature_command.colmap_args
    assert expected_image_path in mapper_command.colmap_args
    assert "rawData" in expected_image_path


def test_no_colmap_manager_attribute_for_image_junction(tmp_path: Path):
    """The junction-path/junction-creation API must be gone entirely,
    not merely unused, so nothing in the codebase can accidentally call
    back into it."""
    project_manager = _open_project(tmp_path)
    manager = COLMAPManager(project_manager)

    assert not hasattr(manager, "images_junction_path")
    assert not hasattr(manager, "ensure_image_junction")


def test_project_setup_does_not_create_colmap_images_folder(tmp_path: Path):
    project_manager = _open_project(tmp_path)
    colmap_dir = project_manager.current.resolver.colmap

    assert not (colmap_dir / "images").exists()
