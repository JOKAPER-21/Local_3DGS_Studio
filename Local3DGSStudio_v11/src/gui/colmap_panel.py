...
class ColmapPanel(QWidget):
    ...
    def __init__(
        ...
        properties_layout.addWidget(status_card)
        ...
        properties_layout.addWidget(input_card)
        ...
        properties_layout.addWidget(settings_card)
        properties_layout.addStretch(1)

        # -- pipeline status (center) ---------------------------------------------------------
        center_layout.addWidget(self._label("COLMAP Pipeline"))
        self.pipeline_widget = ColmapPipelineWidget()
        center_layout.addWidget(self.pipeline_widget)
        ...
        # -- live output (center, fills remaining space) ------------------------------------------------
        center_layout.addWidget(self._label("Live Output"))
        self.log_view = QPlainTextEdit()
        self.log_view.setObjectName("logView")
        self.log_view.setReadOnly(True)
        center_layout.addWidget(self.log_view, stretch=1)

        outer.addWidget(center_column, stretch=1)
        outer.addWidget(properties_column)
    ...
    def _record_step_completion(self, step: str, result: ProcessResult) -> None:
        if step == "Database":
            ...
            # Diagnostic logging (DEBUG-only, so normal runs stay quiet):
            # proves which stream(s) actually carried the metric lines,
            # without permanently dumping full command output at INFO.
            self.logger.debug(
                "model_analyzer stdout (%d chars): %r", len(result.stdout), result.stdout
            )
            self.logger.debug(
                "model_analyzer stderr (%d chars): %r", len(result.stderr), result.stderr
            )
            combined = result.stdout + "\n" + result.stderr
            metrics = parse_model_analyzer_output(combined)
            if not metrics:
                self.logger.warning(
                    "model_analyzer exited 0 but no metrics could be parsed from "
                    "%d combined characters of output.", len(combined)
                )
