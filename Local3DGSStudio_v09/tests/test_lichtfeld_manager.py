...
def test_prepare_scene_stages_without_launching(qt_app, tmp_path, fake_lichtfeld_executable):
    """'Prepare Scene' shares every validation/staging check with
    send_to_lichtfeld, but must never launch the executable."""
    project_manager = _open_project(tmp_path)
    resolver = project_manager.current.resolver
    _write_fake_images(resolver.images, 4)
    _write_fake_sparse_model(resolver.colmap / "sparse" / "0")
    ...
    try:
        result = manager.prepare_scene(_ready_qc_result())
        assert result.success is True
        assert result.staging is not None and result.staging.ok
        assert result.pid == 0  # never launched
    finally:
        cleanup_staging(project_manager.current.data.name, project_manager.current.data.version)


def test_prepare_scene_blocked_when_qc_not_ready(tmp_path, fake_lichtfeld_executable):
    project_manager = _open_project(tmp_path)
    config = LichtFeldConfig(executable_path=str(fake_lichtfeld_executable))
    manager = LichtFeldManager(project_manager, config)

    result = manager.prepare_scene(QCResult(state=QC_STATE_NOT_READY))
    assert result.success is False


def test_prepare_scene_fails_when_executable_missing(tmp_path):
    project_manager = _open_project(tmp_path)
    resolver = project_manager.current.resolver
    _write_fake_images(resolver.images, 4)
    _write_fake_sparse_model(resolver.colmap / "sparse" / "0")

    manager = LichtFeldManager(project_manager, LichtFeldConfig(executable_path=""))
    result = manager.prepare_scene(_ready_qc_result())
