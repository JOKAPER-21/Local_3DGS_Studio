...
class ManagedProcess(QObject):
    ...
    def _on_finished(self, exit_code: int, exit_status: QProcess.ExitStatus) -> None:
        # QProcess can emit `finished` without a preceding readyRead*
        # signal for the very last chunk of output a short-lived process
        # wrote right before exiting -- a known race. A tool like COLMAP's
        # model_analyzer prints its entire metric summary and exits almost
        # immediately, which lands squarely in this window: without this
        # explicit drain, those trailing lines are silently dropped from
        # both buffers even though the process exited 0 and really did
        # write them. Draining both channels here, regardless of which one
        # a given tool actually uses, closes that gap.
        self._drain_stdout()
        self._drain_stderr()
