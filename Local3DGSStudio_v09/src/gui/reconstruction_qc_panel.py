...
class ReconstructionQCPage(QWidget):
    def __init__(
        ...
        self.send_to_lichtfeld_button.setToolTip(
            "Enabled once Reconstruction QC is READY or WARNING."
        )
        self.send_to_lichtfeld_button.clicked.connect(self._on_send_to_lichtfeld)
        ...
        self.lichtfeld_status_label = QLabel("")
        self.lichtfeld_status_label.setWordWrap(True)
        outer.addWidget(self.lichtfeld_status_label)
    ...
    def _result_from_stored(self, stored: dict) -> QCResult:
        return QCResult.from_project_json_dict(stored)
    ...
    def _display_empty(self, message: str) -> None:
        ...
        self.send_to_lichtfeld_button.setEnabled(False)
        self.lichtfeld_status_label.setText("")
    ...
    def _on_send_to_lichtfeld(self) -> None:
        # No QMessageBox here, by design (see module docstring): failures
        # and successes both render into lichtfeld_status_label so this
        # page stays safe to drive from headless/automated tests.
        self.logger.info("Send to LichtFeld button clicked")

        if self._last_result is None or not self._last_result.is_ready_for_lichtfeld:
            self.lichtfeld_status_label.setObjectName("statusError")
            self._set_lichtfeld_status_object_name()
            self.lichtfeld_status_label.setText(
                "Reconstruction QC is not READY or WARNING -- run Refresh QC first."
            )
            return

        settings = self.config_manager.load_settings()
        config = LichtFeldConfig(executable_path=settings.external_software.lichtfeld)
        manager = LichtFeldManager(self.project_manager, config)

        try:
            result = manager.send_to_lichtfeld(self._last_result)
        except ProjectLoadError as exc:
            self.lichtfeld_status_label.setObjectName("statusError")
            self._set_lichtfeld_status_object_name()
            self.lichtfeld_status_label.setText(str(exc))
            return

        if result.success:
            self.lichtfeld_status_label.setObjectName("statusOk")
            self._set_lichtfeld_status_object_name()
            self.lichtfeld_status_label.setText(
                f"LichtFeld Studio started (PID {result.pid}). Staged dataset: "
                f"{result.staging.staging_root if result.staging else 'n/a'}"
            )
        else:
            self.lichtfeld_status_label.setObjectName("statusError")
            self._set_lichtfeld_status_object_name()
            self.lichtfeld_status_label.setText(f"Send to LichtFeld failed: {result.error}")

    def _set_lichtfeld_status_object_name(self) -> None:
        self.lichtfeld_status_label.style().unpolish(self.lichtfeld_status_label)
        self.lichtfeld_status_label.style().polish(self.lichtfeld_status_label)
