...
class ColmapPanel(QWidget):
    ...
    def _begin_run(self) -> bool:
        ...
        junction_result = self.manager.ensure_image_junction()
        if not junction_result.ok:
            QMessageBox.critical(self, "Image Junction Error", junction_result.message)
            self.logger.error("COLMAP image junction error: %s", junction_result.message)
            return False
