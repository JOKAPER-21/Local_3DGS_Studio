...
class ProjectStructureManager:
    ...
    def create_version(
        ...
        """Create the images/COLMAP/LichtFeld directories for a new
        version, leaving every previous version's folders untouched.
        Returns (images_dir, colmap_dir, lichtfeld_dir)."""
        relative_paths = RelativeProjectPaths.for_version(version)
        resolver = ProjectPathResolver(project_root, relative_paths)
        ensure_directory(resolver.images)
        ensure_directory(resolver.colmap)
        ensure_directory(resolver.lichtfeld)
        return resolver.images, resolver.colmap, resolver.lichtfeld
